...
class Strategy(BaseStrategy):
    ...
    def on_step(self, state):
        ...
        # === MONOPOLY (aggressive, same as v74) ===
        if comp_bid is None or comp_ask is None:
            ...
            if comp_bid is None and comp_ask is not None:
                ...
                if ask_tick is not None:
                    ...
                    sell_coll = (1.0 - ask_tick / 100.0) * max(0, ss - state.yes_inventory)
                    if ss >= 0.01 and sell_coll < usable_cash:
                ...
                for tick in range(99, min_tick - 1, -1):
                    ...
                    if ss >= 0.01:
                        sell_coll = (1.0 - tick / 100.0) * ss
                        if sell_coll < usable_cash:
                            actions.append(PlaceOrder(side=Side.SELL, price_ticks=tick, quantity=ss))
                            net_inv -= ss
                            usable_cash -= sell_coll
        ...
        # KEY CHANGE: flat size=10 at moderate z, ramp only at high z
        # At z < 2.5: size=10 (same retail, 5x less arb than size=50)
        # At z >= 2.5: ramp up (arb risk drops exponentially)
        if z < 2.5:
            size = 10.0
        else:
            size = max(10.0, 20.0 * (z - 2.0) ** 2)

        skew_rate = min(0.12, 1.5 / max(3.0, size))
        if net_inv > 0:
